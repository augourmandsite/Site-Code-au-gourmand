...
function ReservationPage() {
  ...
  const submitReservation = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = await response.json() as { error?: string; notificationSent?: boolean }
      if (!response.ok) throw new Error(data.error)
      setSubmitState('success')
      setSubmitMessage(data.notificationSent ? 'Un e-mail de confirmation vient de vous être envoyé.' : 'Nous vous confirmerons la réservation rapidement.')
